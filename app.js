...
const QUIZZES = {
  1: [{
    ...
    id: 'q1c', title: '국회 온톨로지 만들기 (심화)',
    prompt: `국회 회의록을 예로, 아래 조각을 클래스 · 속성(관계) · 공리(제약) 세 칸에 나눠 담아 보세요.
    ...
    zones: [{ z: 'cls', l: '클래스' }, { z: 'prop', l: '속성(관계)' }, { z: 'ax', l: '공리(제약)' }],
    items: [{ i: 'd', l: '~을 낭독했다' }, { i: 'b', l: '회의록' },
    { i: 'h', l: '표결 없이 발언만 있는 회차도 있다' }, { i: 'a', l: '국회의원' },
    { i: 'f', l: '~의 결과로 남았다' }, { i: 'j', l: '회의록 하나는 회의 하나에서만 나온다' },
    { i: 'c', l: '댓글' }, { i: 'e', l: '~을 작성했다' },
    { i: 'g', l: '“작성했다”의 주어는 인물만, 목적어는 기록만' }],
    key: { a: 'cls', b: 'cls', c: 'cls', d: 'prop', e: 'prop', f: 'prop', g: 'ax', h: 'ax', j: 'ax' },
    why: {
      c: '화면에 뜬 댓글도 <b>그 자체로 기록</b>입니다. 회의록에 낭독돼 들어갔다고 회의록의 일부가 되는 것은 아닙니다 — 쓴 사람도, 쓰인 자리도 다릅니다.',
      f: '기록과 사건을 잇는 <b>화살표</b>입니다. ‘회의록’이라는 종류도, 이 회의록이라는 개체도 아닙니다.',
      h: '어떤 값을 가질 수 있는지 못박는 <b>규칙</b>입니다 — 표결이 <b>반드시 있어야 하는 것은 아니다</b>라고 말하는 제약이지요.',
      j: '몇 번까지 이어질 수 있는지를 정하는 <b>카디널리티</b>. 관계에 덧붙는 조건이라 공리입니다.',
    },
    done: '실제 회의록에서도 명시하는 것은 같은 셋입니다 — 다만 여기서는 <b>기록 안에 또 다른 기록</b>이 인용돼 있습니다.',
    learn: [
      { t: '기록 안의 기록 — rico:Record', d: `회의록도 댓글도 <b>각각 하나의 기록</b>입니다. 낭독돼 회의록에 들어갔다고 둘이 한 개체가 되지는 않습니다. 인용한 쪽과 인용된 쪽을 갈라 두어야 “이 말은 누구 것인가”를 되짚을 수 있습니다.` },
      { t: 'rico:hasAuthor', d: `도메인 <b>Record</b> → 레인지 <b>Person · Group …</b>. 댓글의 저자는 <b>‘YD부산댁’</b>이고, 회의록의 저자는 국회입니다. 저자가 다르다는 사실이 두 기록을 갈라 세우는 근거입니다.` },
      { t: 'rico:resultsOrResultedIn', d: `도메인 <b>Event · Activity</b> → 레인지 <b>Record …</b>. “회의록이 회의를 기록한다”를 RiC-O 는 <b>본회의(사건) → 회의록</b> 방향으로 적습니다 — 회의의 <u>결과로</u> 남은 것이라는 뜻입니다.` },
      { t: '공리 — 도메인·레인지, 카디널리티, 값 제약', d: `“작성했다”의 주어는 인물만(도메인), 회의록 하나는 회의 하나에서만(카디널리티), 표결은 없을 수도 있다(값 제약). 셋 다 <b>속성에 붙는 조건</b>이라 공리입니다.` },
    ],
    learnNote: `‘YD부산댁’ 같은 이름은 전거로 세울 수 있을까요? 태어나고 죽는 실재하는 사람이니 <b>전거의 대상</b>이지만, 본명을 모르면 <b>어느 사람인지 확정할 수 없습니다</b> — 8장 전거레코드와 5장 식별자가 바로 이 문제를 다룹니다.`,
  }],
  2: [{
