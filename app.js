...
const QUIZZES = {
  1: [{
    ...
    id: 'q1c', title: '국회 온톨로지 만들기 (심화)', mode: 'pick', numbered: true,
    prompt: `이번에는 조각을 나누는 대신 <b>판단</b>을 해 봅니다. 아래 회의록 한 대목만을 근거로,
      각 진술을 지식그래프에 어떻게 다룰지 고르세요.<br>
      <b>사실로 적는다</b> — 회의록이 직접 보증합니다 ·
      <b>주장으로 적는다</b> — “이런 주장이 인용되었다”까지만 참입니다 ·
      <b>적지 않는다</b> — 기록에 없는 추론입니다.
      <span class="qsrc">제19대국회 제340회(임시회) 제7차 국회본회의(2016. 2. 23.) 회의록</span>
      <span class="qquote">많이들 지루하실 것 같아서……<br>
        요즘 인기 있는 마리텔이라는 게 있더라고요. 그리고 망치부인 같은 그런 것도 있고 그래서
        제가 댓글들을 올라오는 걸 읽어 보도록 하겠습니다.<br>
        YD부산댁이라는 분이신데요 ‘대통령님이 20분간 책상 치셨다는데 안 망가진 것 보니까
        좋은 제품인 것 같아요. 혹시 어떤 책상인지 물어봐 주실 수 있나요?’</span>`,
    zones: [{ z: 'fact', l: '사실로 적는다' }, { z: 'claim', l: '주장으로 적는다' }, { z: 'no', l: '적지 않는다' }],
    items: [
      { i: 'd', v: '대통령이 20분간 책상을 쳤다.', l: '대통령이 20분간 책상을 쳤다' },
      { i: 'a', v: '이 본회의에서 한 의원이 댓글들을 낭독했다.', l: '의원이 댓글들을 낭독했다' },
      { i: 'g', v: '그 책상은 두드려도 안 망가지는 좋은 제품이다.', l: '그 책상은 좋은 제품이다' },
      { i: 'e', v: '‘YD부산댁’이라는 계정이 실제로 이 댓글을 작성했다.', l: '‘YD부산댁’이 실제로 이 댓글을 작성했다' },
      { i: 'b', v: '회의록에 ‘YD부산댁’ 명의로 소개된 댓글 텍스트가 실려 있다.', l: '회의록에 그 댓글 텍스트가 실려 있다' },
      { i: 'h', v: '‘YD부산댁’은 부산에 사는 여성이다.', l: '‘YD부산댁’은 부산에 사는 여성이다' },
      { i: 'f', v: '이 댓글은 실제로 온라인 방송 플랫폼에 게시되었던 것이다.', l: '댓글이 실제로 게시되었던 것이다' },
      { i: 'c', v: '발언 중에 ‘마리텔’과 ‘망치부인’이라는 이름이 언급되었다.', l: '‘마리텔’·‘망치부인’이 언급되었다' },
      { i: 'k', v: '대통령은 그 무렵 화가 나 있었다.', l: '대통령은 화가 나 있었다' }],
    key: { a: 'fact', b: 'fact', c: 'fact', d: 'claim', e: 'claim', f: 'claim', g: 'no', h: 'no', k: 'no' },
    why: {
      d: '회의록이 보증하는 것은 <b>이런 주장이 낭독되었다</b>는 것까지입니다. 정말 쳤는지는 이 기록 밖의 일이니, 사건이 아니라 <b>주장된 사건</b>으로 세웁니다.',
      e: '귀속조차 전언입니다 — 의원이 그렇게 <b>소개했다</b>는 것만 확실하고, 원댓글은 확보되지 않았습니다. “~에게 귀속된다고 인용되었다”로 적습니다.',
      f: '원문이 확인되지 않았습니다. 게시 여부·플랫폼·시각 모두 <b>발언이 주장하는 내용</b>이라, 출처 참조로만 세워 둡니다.',
      g: '<b>풍자</b>입니다. 문장의 기능(풍자·반어)을 접어 두고 제품 평가로 옮겨 적으면, 농담이 데이터가 되어 버립니다.',
      h: '표시명에서 거주지와 성별을 <b>추론</b>한 것입니다. 화면의 이름은 이름일 뿐, 실명 정체성과 같지 않습니다.',
      k: '기록 어디에도 없는 <b>심리 추론</b>입니다. 책상을 쳤다는 것 자체가 미검증 주장인데, 거기서 감정까지 끌어내면 추론 위에 추론을 쌓게 됩니다.',
      b: '이건 회의록이 <b>직접 보증</b>합니다 — 인용 텍스트가 전사되어 실려 있다는 것은 기록 자체의 사실입니다. 내용의 참·거짓과는 별개입니다.',
      c: '언급이 <b>있었다는 것</b>은 회의록에 그대로 적혀 있습니다. 그 방송이 정말 인기였는지는 별개의 문제고요.',
    },
    hints: ['판단 기준 — 이 회의록이 보증하는 것은 “그 자리에서 이 텍스트가 낭독되고 기록되었다”까지입니다. 인용문 <b>내용</b>의 참·거짓은 회의록 밖의 일입니다.'],
    done: '경계선을 정확히 그었습니다. 회의록이 보증하는 것과 회의록 속 인용이 주장하는 것을 가르는 일 — 이것이 이 발췌로 온톨로지를 설계할 때의 핵심입니다.',
    learn: [
      { t: '인용은 사실이 아니다', d: `회의록에 인용되었다는 사실은 인용문 내용이 사실이라는 뜻이 아닙니다. 그래서 “책상을 쳤다”는 사건으로 적지 않고 <b>주장된 사건</b>으로 세우고, 주장에는 <b>검증 상태</b>(미검증·확인·반박)를 함께 답니다.` },
      { t: '표시명은 실명이 아니다', d: `‘YD부산댁’은 화면에 나타난 이름일 뿐입니다. 실명 개인으로 단정하지 않고 <b>가명 계정</b>으로 세워 두면, 나중에 본인이 확인되었을 때 계정에 사람을 이어 붙이면 됩니다 — 처음부터 사람으로 적으면 되돌리기 어렵습니다.` },
      { t: '풍자는 속성이 아니다', d: `“좋은 제품인 것 같아요”를 책상의 품질로 옮겨 적으면 안 됩니다. 문장의 <b>기능</b>(풍자·질문·비판)을 함께 적어 두어야, 뒤에 읽는 기계가 농담을 제품 정보로 오해하지 않습니다.` },
      { t: '주장도 개체로 세운다', d: `이 세 원칙을 그래프로 구현하는 길은 하나입니다 — 주장·인용행위·계정을 <b>각각 개체로</b> 세우고 관계로 잇는 것. 회의·발언·주장 같은 클래스는 RiC-O 에 그대로 있지 않아, 실제 설계에서는 <code>rico:Event · Activity · Record · Agent</code> 아래에 기관 고유 어휘를 새로 답니다 — 7장 RDFS·OWL 이 그 방법을 다룹니다.` },
    ],
    learnNote: `기록학이 늘 해 온 일이기도 합니다 — 기록은 <b>진본성</b>(이 문서가 그 자리에서 만들어졌다)을 보증할 뿐, 내용의 <b>진실성</b>까지 보증하지 않습니다. 온톨로지는 그 구분을 기계도 읽을 수 있게 적는 방법입니다.`,
  }],
  2: [{
